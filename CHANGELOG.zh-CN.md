...
- 架构交接以规范 Markdown 中的设计、用户确认和证据为来源，LikeC4 为可选结构模型。
- 分离设计确认、实施进度和验证结论，不再规定 C4 批准字段或固定指纹算法。
- 不改变 handoff/OpenSpec 运行时结构；历史 C4 夹具保留为历史场景，不宣称覆盖新规则。
- 发布验收记录见仓库 docs/releases/1.7.4.md 及 GitHub Release；安装包仅包含运行插件所需内容。

## 1.7.3（2026-09-10）

- 初始化和同步在索引缺失时检查 current、history 与旧 records 存储，避免创建空索引遮蔽已有交接。
- 同步在写入 context、索引或授权前完成 AGENTS 受管内容校验；不把单文件原子写入表述为跨文件事务。
- 匹配与历史查询的自动索引修复统一持有项目写锁，并在锁内读取当前状态。
- 适配 Sol Advisor 2.0：主会话核对子代理证据后按准入规则保存长期记录；移除旧 Context Analyst 引用。
- 验证：48 项相关源码运行测试通过；随后 TypeScript 构建、本地打包和 Codex 1.7.3 插件安装通过，直接使用已安装产物的 12 项针对性测试通过。未执行完整发布测试套件。

## 1.7.2（2026-09-08）

- 同步任务授权、依赖范围暂停及最小验证规则；OpenSpec 普通错误在既有范围内恢复，依赖内容按变化重读。
- 收窄技能描述；初始化和同步采用一次结果回读，重复提交幂等检查留在插件回归测试。
- 保留项目策略、管理边界和已有幂等测试；本次不改变运行时合同。

## 1.7.1（2026-09-05）

- 明确允许委派不等于必须委派；模型与角色选择由 Advisor 管理，不可用或额度不足时由主窗口检查已有工作后接续。
- RAG 证据充分时直接使用，已定位长资料仅在存在独立提取需求时进入 Context Analyst。
- 91 项测试及真实新会话 Hook 注入、同修订去重、修订更新、Embedded 阻塞恢复回归通过。

## 1.7.0（构建、91 项测试与候选安装验证通过）

- 写锁改为完整所有者信息原子发布的不可变代际记录；存活或无法确认死亡的所有者不会因超时被接管，已确认退出的本机所有者可安全恢复，旧版单文件锁仅在确认死亡后迁移。
- Plan 查询通过现有 `warnings` 返回指向不存在 Plan 的 Change ID、`planId` 与文件位置；Hook 增加并发预约、输出失败释放和死亡所有者恢复检查。
- 评审修订：Handoff 索引新写入升级 v5，读取兼容 v3/v4；只读访问不因版本升级重写有效索引，旧 current/history Markdown 保持可读。
- Hook 在会话内原子预约输出，保留进程退出后的重试机会；按 hooks.json 的总上限计算完整上下文，未输出卡片保持可用。stdout 与状态落盘间中断仍可能重发。
...
- TypeScript 构建与 91 项测试通过；候选插件安装、独立 Codex 客户端加载和默认提示清单检查通过。正式资产从合入后的干净 `main` 提交生成。
