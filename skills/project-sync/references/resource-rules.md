...
- creation of a missing empty schema v5 handoff index only when no handoff storage exists; synchronization never rewrites current, history, or legacy Markdown records.
...
For selected architecture or hardware resources, load only the corresponding [Architecture](../../project-init/references/architecture-context.md) or [Hardware](../../project-init/references/hardware-context.md) reference. Existing Change documents use [Change routing](../../project-init/references/change-management.md). These conditional references own their detailed lifecycle rules.
...
Accept schema-v3/v4 handoff indexes as compatibility input and schema v5 as the current index format. Reject other schemas. Synchronization must not migrate or rewrite existing handoff formats.
