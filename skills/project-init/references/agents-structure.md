...
Keep `Sol Advisor Integration` minimal: state the effective project policy (`inherit`, explicit allow, or explicit deny), the three-state authorization semantics, fail-closed invalid-state behavior, and ownership boundary. Eligibility never requires delegation. The primary uses Project Context workflows for durable writes after checking evidence; children return results without writing managed context. Child completion does not itself create a handoff or transition a project plan. Unavailable or quota-limited routes return ownership to the primary after existing work is checked. Route details, role prompts, model selection, messaging capability and result protocols remain in the Sol Advisor Skill rather than the project file.
