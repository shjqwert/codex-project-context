...
Project Context owns workflows for the project-managed `AGENTS.md` section, `.agent/context.json`, `.agent/authorizations.json`, `.agent/planMsg.md`, and `.agent/handoff/`. It never writes the user-level `~/.codex/AGENTS.md`. Sol Advisor owns current-task delegation, model selection, communication and integration; eligibility does not require delegation. The primary passes only relevant context, locators and current restrictions to children, especially when they start without inherited history. Children return ordinary results; the primary verifies them and uses Project Context workflows only when a durable plan transition or cross-task handoff meets its admission rule. Child completion, messages and temporary ownership are not persistent handoff records or project-plan states.

Communication capability belongs to Sol Advisor and the host: direct messages when available, otherwise evidence relayed by the primary. Project Context does not probe model messaging, store mailboxes or pin role/model names in generated project rules. An unavailable or quota-limited route returns work to the primary after child ownership ends and existing changes are inspected. RAG evidence is used directly when sufficient; an identified long artifact may justify a separate bounded Scout task selected by Advisor.
